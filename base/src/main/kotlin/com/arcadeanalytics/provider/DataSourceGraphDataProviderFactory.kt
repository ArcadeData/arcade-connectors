...
class DataSourceGraphDataProviderFactory(pluginPath: String = "./plugins") {

    private val log = LoggerFactory.getLogger(DataSourceGraphDataProviderFactory::class.java)

    private val dataProviders: MutableMap<String, DataSourceGraphDataProvider> = mutableMapOf()

    init {
        val walk = Files.walk(Paths.get(pluginPath))

        val clazz = DataSourceGraphDataProvider::class.java
        walk.asSequence()
    ...
    fun create(dataSourceInfo: DataSourceInfo): DataSourceGraphDataProvider {
        ...
        if (dataSourceInfo.remote) return SshTunnelDataProviderDecorator(this)
