...
public class DBSourceConnection {
  ...
  private static Map<String, String> type2template =
      new HashMap<String, String>() {
        {
          ...
          put(
              "RDBMS_MYSQL",
              "jdbc:mysql://{server}:{port}/{database}?nullNamePatternMatchesAll=true&autoReconnect=true&useSSL=false");
  ...
  public static Connection getConnection(DataSourceInfo datasource) {
    ...
    try {
      Map<String, String> connectionAdditionalProperties =
          new ObjectMapper()
              .readValue(
                  Optional.ofNullable(datasource.getConnectionProperties()).orElse("{}"),
                  HashMap.class);
