...
/**
 * Extends ER2GraphMapper thus manages the source DB schema and the destination graph model with their correspondences.
...
public class Hibernate2GraphMapper extends ER2GraphMapper {
  ...
  public void buildSourceDatabaseSchema() {
    try {
      ...
      for (int i = 0; i < entities.getLength(); i++) {
        ...
        if (currentEntityElement.hasAttribute("table")) currentEntity =
          super.dataBaseSchema.getEntityByNameIgnoreCase(currentEntityElement.getAttribute("table")); else {
        ...
        // inheritance
        if (currentEntity != null) this.detectInheritanceAndUpdateSchema(currentEntity, currentEntityElement);
      }
  ...
  private void detectInheritanceAndUpdateSchema(Entity parentEntity, Element parentEntityElement) {
    ...
    if (subclassElements.getLength() > 0) {
      if (parentEntityElement.hasAttribute("discriminator-value")) rootDiscriminatorValue = parentEntityElement.getAttribute("discriminator-value");
  ...
  private void performSubclassTagInheritance(
    ...
    // Table per Class Hierarchy
      ...
      for (int i = 0; i < subclassElements.getLength(); i++) {
        ...
        if (currentEntityElement.hasAttribute("name")) currentEntityElementName = currentEntityElement.getAttribute("name"); else {
        ...
        for (int j = 0; j < propertiesElements.getLength(); j++) {
          ...
          currentChildAttribute =
            new Attribute(currentParentCorrespondingAttribute.getName(), j + 1, currentParentCorrespondingAttribute.getDataType(), currentChildEntity);
        ...
        if (hierarchicalBag.getDepth2entities().get(currentChildEntity.getInheritanceLevel()) == null) {
          ...
          Set<Entity> tmp = hierarchicalBag.getDepth2entities().get(currentChildEntity.getInheritanceLevel());
          tmp.add(currentChildEntity);
          hierarchicalBag.getDepth2entities().put(currentChildEntity.getInheritanceLevel(), tmp);
        }
  ...
  private void performJoinedSubclassTagInheritance(HierarchicalBag hierarchicalBag, Entity parentEntity, NodeList joinedSubclassElements) {
    ...
    for (int i = 0; i < joinedSubclassElements.getLength(); i++) {
      ...
      if (currentChildElement.hasAttribute("table")) currentChildEntityName = currentChildElement.getAttribute("table"); else {
        log.error("XML Format error: problem in subclass definition, table attribute missing in the joined-subclass nodes.\n");
        throw new RDBMSProviderRuntimeException();
      }
